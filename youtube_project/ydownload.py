...
class DownloadForm(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def add_text(self, row, column, text):
        if text is None:
            return
    ...
    def parse_link(self):
        ...
        try:
            self.yt = pytube.YouTube(link, proxies={'https': ip_port_text})
            streams = self.yt.fmt_streams
            length = self.yt.length
        except pytube.exceptions.RegexMatchError:
            self.status_update('Invalid link.')
            return
        except pytube.exceptions.VideoUnavailable:
            self.status_update('Video unavailable.')
            return
        ...
        for i in range(source_count):
            ...
            if stream.type == 'video':
                ...
                self.add_text(i, 4, f'{int(stream.filesize_approx / 128 / length)}')
            else:
                self.add_text(i, 4, stream.abr.split('kbps')[0])
            self.add_text(i, 0, stream.resolution)
            self.add_text(i, 2, stream.subtype)
